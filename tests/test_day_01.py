import pathlib
import pytest
from day_01 import day_01 as aoc
...
def example1():
    print(PUZZLE_DIR)
    puzzle_input = (PUZZLE_DIR / "day_01/example1.txt").read_text().strip()
    return aoc.parse(puzzle_input)

@pytest.fixture
def example2():
    puzzle_input = (PUZZLE_DIR / "day_01/example2.txt").read_text().strip()
    return aoc.parse(puzzle_input)
